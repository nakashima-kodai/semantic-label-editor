...
def show_loaded_image(label_tensor, truth_tensor):
    ...
    labeltruth = torchvision.utils.make_grid(labeltruth, nrow=2*4, padding=1)
...
def save_images(opt, epoch, label, truth, fake_image):
    ...
    lbltrufake = torchvision.utils.make_grid(lbltrufake, nrow=12, padding=1)
...
def print_current_losses(epoch, i, losses, t):
    message = '(epoch: {:03d}, iters: {:03d}, time: {:.3f}) '.format(epoch, i, t)
